...
export default function ExpedicaoPage() {
  ...
  const speakZion = async (text) => {
    if (!text?.trim() || speaking) return;
    setSpeaking(true);
    try {
      const response = await fetch('/api/zion/speech', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text, language: universe?.language || 'Français' }),
      });
      ...
      if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
        ...
        utterance.lang = speechLanguages[universe?.language] || 'fr-FR';
  ...
  const startListening = () => {
    ...
    if (!SpeechRecognition) {
      setError('Seu navegador não oferece reconhecimento de voz. Você ainda pode escrever sua resposta.');
      return;
    }
    const recognition = new SpeechRecognition();
    recognition.lang = speechLanguages[universe?.language] || 'fr-FR';
    ...
    recognition.onerror = () => {
      setError('Não consegui ouvir sua voz. Tente novamente ou escreva sua resposta.');
      setListening(false);
    };
  ...
  const talkToZion = async () => {
    ...
    try {
      ...
      const nextReply = data.reply || 'Estou aqui. Vamos continuar nossa expedição?';
      setReply(nextReply);
      setConversation((current) => [...current, { role: 'user', text: message }, { role: 'zion', text: nextReply }]);
      setAnswer('');
      if (conversation.length >= 2) setCompleted(true);
    } catch (err) {
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-5 py-12"><div className="w-full max-w-5xl">
          ...
          <AnimatePresence mode="wait">
            ...
            {mission&&!completed&&<motion.section key="mission" initial={{opacity:0,y:25}} animate={{opacity:1,y:0}} className="mx-auto max-w-4xl"><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] items-center"><div className="text-center lg:text-left"><div className="mx-auto lg:mx-0 h-44 w-44 rounded-full border border-amber-300/30 bg-gradient-to-br from-amber-200/10 to-purple-500/10 grid place-items-center"><span className="text-7xl">👾</span></div><p className="mt-6 text-[10px] uppercase tracking-[.38em] text-amber-200/70">Zion · seu guia</p></div><div><div className="rounded-3xl border border-white/10 bg-white/[.035] p-7 md:p-9"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 text-amber-200/70 text-[10px] uppercase tracking-[.28em]"><Volume2 size={14}/> Primeira conversa</div><span className="text-[10px] uppercase tracking-[.2em] text-white/30">{Math.min(3, Math.floor(conversation.length / 2) + 1)} / 3</span></div><h2 className="mt-5 font-serif text-4xl md:text-5xl font-light">Oi. Eu sou o Zion.</h2><p className="mt-5 text-lg text-white/70 font-light">Vamos descobrir até onde você consegue chegar?</p><button onClick={()=>speakZion('Oi. Eu sou o Zion. Vamos descobrir até onde você consegue chegar?')} disabled={speaking} className="mt-7 rounded-full border border-white/10 p-3 disabled:opacity-40" aria-label="Ouvir Zion"><Volume2 size={18}/></button></div>
              {conversation.length>0&&<div className="mt-5 space-y-3">{conversation.slice(-4).map((item,index)=><div key={`${item.role}-${index}`} className={`rounded-2xl border p-4 ${item.role==='zion'?'border-amber-300/10 bg-white/[.035]':'border-purple-300/15 bg-purple-500/[.05]'}`}><p className="text-[9px] uppercase tracking-[.25em] text-white/30">{item.role==='zion'?'Zion':'Você'}</p><p className="mt-2 text-sm leading-relaxed text-white/70">{item.text}</p>{item.role==='zion'&&<button onClick={()=>speakZion(item.text)} disabled={speaking} className="mt-3 text-white/45 hover:text-amber-200 disabled:opacity-30" aria-label="Ouvir resposta do Zion"><Volume2 size={15}/></button>}</div>)}</div>}
              <div className="mt-5 rounded-2xl border border-purple-300/20 bg-purple-500/[.06] p-5"><p className="text-xs uppercase tracking-[.25em] text-purple-200/70">Sua vez</p><p className="mt-2 text-white/65">Responda em {universe?.language||'no idioma'} como preferir. O Zion vai ouvir, entender e continuar a conversa.</p><div className="mt-4 flex gap-3"><button onClick={startListening} className={`grid h-12 w-12 shrink-0 place-items-center rounded-xl ${listening?'bg-amber-500 text-black':'bg-purple-600/80'}`} aria-label="Falar com Zion"><Mic size={19}/></button><input value={answer} onChange={e=>setAnswer(e.target.value)} onKeyDown={e=>e.key==='Enter'&&talkToZion()} placeholder="Escreva sua resposta..." disabled={loading} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-4 outline-none disabled:opacity-60"/><button onClick={talkToZion} disabled={!answer.trim()||loading} className="rounded-xl bg-white px-5 text-black disabled:opacity-30"><ArrowRight size={18}/></button></div>{listening&&<p className="mt-3 text-xs text-amber-200/70 animate-pulse">Estou ouvindo…</p>}{loading&&<p className="mt-3 text-xs text-amber-200/70 animate-pulse">Zion está ouvindo…</p>}{error&&<p className="mt-3 text-xs text-red-300/80">{error}</p>}</div></div></div></motion.section>}
            {completed&&mission&&<motion.section key="complete" initial={{opacity:0,scale:.94}} animate={{opacity:1,scale:1}} className="mx-auto max-w-3xl text-center"><div className="mx-auto h-24 w-24 rounded-full border border-amber-300/50 bg-amber-200/10 grid place-items-center"><Zap className="text-amber-200" size={38}/></div><p className="mt-7 text-[10px] uppercase tracking-[.4em] text-amber-200/70">Primeiro passo concluído</p><h2 className="mt-4 font-serif text-5xl md:text-6xl font-light">Você começou.</h2>{reply&&<div className="mx-auto mt-6 max-w-2xl rounded-3xl border border-purple-300/20 bg-purple-500/[.06] p-6 text-left"><div className="flex items-center justify-between gap-3"><p className="text-[10px] uppercase tracking-[.28em] text-purple-200/70">Zion respondeu</p><button onClick={()=>speakZion(reply)} disabled={speaking} className="text-white/45 disabled:opacity-30" aria-label="Ouvir Zion"><Volume2 size={17}/></button></div><p className="mt-3 text-lg leading-relaxed text-white/80 font-light">{reply}</p></div>}<p className="mt-5 text-white/60">O Zion já registrou sua primeira interação na expedição.</p><div className="mx-auto mt-8 flex max-w-sm justify-center gap-3"><div className="rounded-2xl border border-amber-300/20 bg-amber-200/[.06] px-7 py-5"><div className="text-3xl font-serif">+10</div><div className="mt-1 text-[10px] uppercase tracking-[.25em] text-white/40">XP</div></div><div className="rounded-2xl border border-purple-300/20 bg-purple-500/[.06] px-7 py-5"><div className="text-3xl font-serif">+3</div><div className="mt-1 text-[10px] uppercase tracking-[.25em] text-white/40">ZAPS</div></div></div><button onClick={()=>{setCompleted(false);setMission(false);setReply('');setAnswer('');setConversation([]);setStep(5)}} className="mt-9 inline-flex items-center gap-3 rounded-xl bg-white px-8 py-4 text-xs font-semibold uppercase tracking-[.22em] text-black">Voltar ao mapa <ArrowRight size={16}/></button></motion.section>}
          </AnimatePresence>
        </div></div>
      </div>
    </main>
